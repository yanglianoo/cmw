        ...
        const CacheChange_t* const change)
{
    ...
    char * addr = (char*)change->serializedPayload.data;
    size_t len = change->serializedPayload.length;
    
    


    std::string data(addr,len);

    DataStream ds((char*)data.data() ,data.size());

    int index;

    ds >> index ;
    std::cout <<"index :" << index <<"length ds: "<< len <<"length data:" << data.size() << std::endl;
